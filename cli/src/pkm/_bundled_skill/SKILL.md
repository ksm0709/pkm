...
mcp__pkm__get_note_neighbors(note_id="note-slug")
mcp__pkm__read_note(note_id="note-slug")
...
3. Before synthesizing a cross-note answer, call
   `mcp__pkm__read_note(note_id=<slug>)` for every selected source. The agent host
   synthesizes only after reading the evidence; graph traversal stops at two
   depths total.
...
### B. During Work: Evidence-Based Synthesis

When a question arises about prior decisions, patterns, or preferences, use the
direct host-side sequence: `search` → `get_note_neighbors` → `read_note` → host
synthesis. Traverse at most two graph depths, and never synthesize from search
snippets alone: explicitly read the notes selected as evidence.
