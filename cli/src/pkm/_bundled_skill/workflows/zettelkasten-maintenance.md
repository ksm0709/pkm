# Zettelkasten Maintenance Procedure

## Purpose
Execute this deterministic, manual tool-driven routine to maintain knowledge graph health, connect isolated thoughts, and refine notes. Start it only when a user or agent host explicitly chooses to perform maintenance.

The procedure keeps capture and promotion separate: `daily/` stores time-bound
memory, while `notes/` stores durable wiki-style knowledge with explicit
wikilinks and typed relation markers.

## Start Condition

- Run manually when reviewing vault health or consolidating eligible daily notes.
- Confirm the target vault before the first read or write.
- Do not assume a background schedule or automatic execution.
...
4. **Auto-Linking And Relation Repair**
   - Use `search` and inspect graph neighbors (`get_note_neighbors`, `find_surprising_connections`, `list_clusters`, `list_god_nodes`) to find related notes that are not currently linked via `[[wikilinks]]`.
