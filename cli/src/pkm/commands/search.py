...
def index_cmd(ctx: click.Context) -> None:
    ...
    console.print(
        f"[green]✓ AST cache updated[/green] [dim]({vault.pkm_dir / 'ast.db'})[/dim]"
    )
    console.print(
        f"[green]✓ Structural graph built[/green] [dim]({vault.pkm_dir / 'graph.json'})[/dim]"
    )
    console.print(
        f"[green]✓ Vector cache updated[/green] [dim]({vault.pkm_dir / 'vector.db'})[/dim]"
    )
    console.print(
