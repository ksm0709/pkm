...
async def handle_ask(
    ...
    system_prompt = (
        ...
        "- create_daily_subnote(title, content): create structured time-bound daily material\n"
        ...
        "- add_note(title, content, tags, memory_type, importance): create a durable atomic note\n"
        "- patch_note(note_id, operation, old, new, section, fields): partial note edits\n"
        "- update_note(note_id, content, tags): replace an existing note's full body\n"
        "- rename_note(old_note_id, new_note_id): rename a note and rewrite backlinks\n"
        "- get_note_neighbors(note_id, include_semantic): get inbound/outbound/tag and optional semantic graph context\n"
        ...
        "Memory policy: use add_daily_log for time-bound session state, progress, event logs, temporary TODOs, and current-task status. Use create_daily_subnote for structured time-bound material such as meetings, investigations, and long session notes. Use add_note only for durable knowledge with a stable definition, long-term scope, and at least one relation or source link; do not create ordinary episodic/session-state notes under notes/.\n"
        "Relation policy: for durable notes, prefer typed `&relation [[target]] - reason` markers using built-in labels such as is_a, part_of, depends_on, enables, contrasts_with, supersedes, instance_of, related, or source. Treat daily relation markers as promotion candidates only, not canonical graph relations.\n"
