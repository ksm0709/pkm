...
def test_session_start_plain(runner, vault_env):
    ...
    result = runner.invoke(main, ["agent", "hook", "session-start", "--format", "plain"])
...
def test_turn_start_with_session(runner, vault_env):
    ...
    result = runner.invoke(main, ["agent", "hook", "turn-start", "--session", "abc-123"])
    assert result.exit_code == 0
...
def test_turn_end_appends_to_existing_daily_note(runner, vault_env, tmp_vault: VaultConfig):
    ...
    result = runner.invoke(main, ["agent", "hook", "turn-end", "--summary", "finished task X"])
    assert result.exit_code == 0
...
def test_turn_end_creates_daily_note_if_missing(runner, vault_env, tmp_vault: VaultConfig):
    ...
    result = runner.invoke(main, ["agent", "hook", "turn-end", "--summary", "completed work Y"])
    assert result.exit_code == 0
...
def test_turn_end_includes_session_tag(runner, vault_env, tmp_vault: VaultConfig):
    ...
    result = runner.invoke(
        main,
        ["agent", "hook", "turn-end", "--session", "sess-42", "--summary", "did something"],
    )
    assert result.exit_code == 0
    today = date.today().isoformat()
...
def test_turn_end_timestamp_format(runner, vault_env, tmp_vault: VaultConfig):
    ...
    result = runner.invoke(main, ["agent", "hook", "turn-end", "--summary", "timestamp test"])
    assert result.exit_code == 0
    today = date.today().isoformat()
    content = (tmp_vault.daily_dir / f"{today}.md").read_text(encoding="utf-8")
...
def test_setup_hooks_claude_code_merges_existing(runner, vault_env, tmp_path, monkeypatch):
