...
def test_setup_new_vault(tmp_path: Path, runner: CliRunner, monkeypatch) -> None:
    ...
    monkeypatch.setattr("pkm.commands.setup.discover_vaults", lambda root=None: {})
    monkeypatch.setattr("pkm.commands.setup._load_setup_choices", lambda: None)
    monkeypatch.setattr("pkm.commands.setup._save_config_merged", mock_save)
    monkeypatch.setattr("pkm.commands.setup.init_vault_dirs", mock_init)
    monkeypatch.setattr("pkm.commands.setup.subprocess.run", mock_run)
    monkeypatch.setattr("pkm.commands.setup.install_shell_aliases", mock_aliases)
...
def test_setup_existing_vault(tmp_path: Path, runner: CliRunner, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "pkm.commands.setup.discover_vaults", lambda root=None: existing
    )
    monkeypatch.setattr("pkm.commands.setup._load_setup_choices", lambda: None)
    monkeypatch.setattr("pkm.commands.setup._save_config_merged", mock_save)
    monkeypatch.setattr("pkm.commands.setup.init_vault_dirs", mock_init)
