...
- Dream 워크플로우 (`workflows/extract-note-from-daily.md`) — 실제 원자 노트 추출
...
- Dream이 중간에 실패해도 `consolidated: true`는 설정되지 않음 → 다음 실행 시 재시도
...
# 3. 마킹 후 extract-note-from-daily 워크플로우 실행
# → workflows/extract-note-from-daily.md 참고
```
