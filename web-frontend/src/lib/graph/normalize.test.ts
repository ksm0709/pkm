  ...
  it('exposes confidence weight community hub radius and importance from enriched graph data', () => {
    const graph = normalizeGraph({
      nodes: [
        ...
        { id: 'project-plan', title: 'Project Plan', type: 'note', clusters: ['planning'] },
    ...
    expect(graph.nodes.find((node) => node.id === 'hub-pkm-development')).toMatchObject({
      community: 'pkm',
      degree: 1,
      hub: true,
      importance: 1,
    ...
    expect(graph.nodes.find((node) => node.id === 'project-plan')).toMatchObject({
      community: 'planning',
      degree: 2,
      hub: false
    });
