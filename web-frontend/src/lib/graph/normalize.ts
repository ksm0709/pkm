...
export function normalizeGraph(raw: unknown): NormalizedGraph {
  ...
  return {
    ...
    nodeTypes: uniqueSorted(
      nodesWithDegree.map((node) => node.type || "unknown"),
    ),
...
function normalizeNode(entry: unknown): NormalizedGraphNode | null {
  ...
  const id =
    stringFrom(entry.id) ||
    stringFrom(entry.key) ||
    stringFrom(entry.name) ||
    stringFrom(entry.label);
  if (!id) return null;
  const type =
    stringFrom(entry.type) ||
    stringFrom(entry.node_type) ||
    stringFrom(entry.kind) ||
    "note";
  const label =
    stringFrom(entry.title) ||
    stringFrom(entry.label) ||
    stringFrom(entry.name) ||
    id;
  const cluster =
  ...
  return {
    ...
    type,
    description:
      stringFrom(entry.description) || stringFrom(entry.summary) || "",
    community: cluster,
    tier,
    raw: entry,
    degree: 0,
...
function normalizeEdge(
  ...
  const type =
    stringFrom(entry.type) ||
    stringFrom(entry.edge_type) ||
    stringFrom(entry.relation) ||
    stringFrom(entry.kind) ||
    "link";
  return {
...
function confidenceFrom(entry: Record<string, unknown>): number {
  return clamp01(
    numberFrom(entry.confidence) ??
      numberFrom(entry.score) ??
      numberFrom(entry.similarity) ??
      0.5,
  );
}
...
function weightFrom(entry: Record<string, unknown>, type: string): number {
  ...
  if (type === "has_tag" || type === "tagged_by" || type === "tag_note")
    return 0.9;
