...
describe("graph force simulation", () => {
  it("maps semantic confidence and tag links to distinct target distances", () => {
    expect(
      edgeTargetDistance(edge("a", "b", "semantic_similar", 0.95, 1.4)),
    ).toBeLessThan(
      edgeTargetDistance(edge("a", "b", "semantic_similar", 0.2, 1.4)),
    );
    expect(
      edgeTargetDistance(edge("a", "tag:x", "has_tag", 1, 0.9)),
    ).toBeGreaterThan(edgeTargetDistance(edge("a", "b", "wikilink", 1, 1)));
  });

  it("scales node radius by type, degree, and hub status", () => {
    expect(
      simulationNodeRadius(node("hub", "Hub", "note", 8, true, "x")),
    ).toBeGreaterThan(
      simulationNodeRadius(node("leaf", "Leaf", "note", 1, false, "x")),
    );
    expect(
      simulationNodeRadius(node("tag:x", "#x", "tag", 1, false, "x")),
    ).toBeGreaterThanOrEqual(9);
  });

  it("creates deterministic seeded positions and keeps coordinates finite while settling", () => {
    const a = createGraphSimulation(graph, {
      width: 800,
      height: 500,
      seed: "same-seed",
      autoStart: false,
    });
    const b = createGraphSimulation(graph, {
      width: 800,
      height: 500,
      seed: "same-seed",
      autoStart: false,
  ...
  it("does not clamp graph nodes to the visible viewport rectangle", () => {
    const sim = createGraphSimulation(graph, {
      width: 800,
      height: 500,
      seed: "free-world",
      autoStart: false,
