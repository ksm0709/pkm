...
type TestApi = {
  ...
  getFocusState: (
    id: string,
  ) => "focused" | "neighbor" | "muted" | "normal" | null;
...
test.describe("vault graph page", () => {
  test("renders a canvas force graph from enriched graph data", async ({
    ...
    const surfaceFrame = await page
      .getByTestId("graph-force-surface")
      .evaluate((element) => {
  ...
  test("uses semantic confidence, zooms, pans, and drags nodes", async ({
    page,
  }) => {
    const vaultName = TEST_VAULT;
    await mockGraphApi(page, async (route) =>
      json(route, enrichedGraphFixture),
    );

    await page.goto(graphPath(vaultName));
    await settleGraph(page);

    const semantic = await graphEdge(
      page,
      "project-plan",
      "architecture",
      "semantic_similar",
    );
    const wikilink = await graphEdge(
      page,
      "project-plan",
      "journal",
      "wikilink",
    ...
    expect(semantic?.distance ?? Infinity).toBeLessThan(
      wikilink?.distance ?? 0,
    );

    const generationBeforeForceChange = (await graphSimulationState(page))
      .generation;
    ...
    await expect
      .poll(async () =>
        distance(nodeBeforeForceChange!, (await graphNode(page, "journal"))!),
      )
      .toBeGreaterThan(4);
    await settleGraph(page, 80);
    const expandedWikilink = await graphEdge(
      page,
      "project-plan",
      "journal",
      "wikilink",
    ...
    expect(expandedWikilink?.distance ?? 0).toBeGreaterThan(
      wikilink?.distance ?? Infinity,
    );
    ...
    await page.evaluate(async () => {
      await (
        window as typeof window & { __pkmGraphTest: TestApi }
      ).__pkmGraphTest.dragNode("journal", 45, 10);
    });
    const afterNode = await graphNode(page, "journal");
    expect(distance(beforeNode!, afterNode!)).toBeGreaterThan(10);
  });

  test("click focuses a neighborhood without URL navigation", async ({
    page,
  }) => {
    const vaultName = TEST_VAULT;
    await mockGraphApi(page, async (route) =>
    ...
    await expect(page.getByTestId("graph-focus-status")).toContainText(
      "Project Plan",
    );
    await expect(page.getByTestId("graph-focus-description")).toContainText(
      "Planning hub for active PKM work.",
    );
  ...
  test("double tapping empty graph space clears the focused neighborhood", async ({
    page,
  }) => {
    const vaultName = TEST_VAULT;
    await mockGraphApi(page, async (route) =>
  ...
  test("cmd click and long press open note preview while tag and unresolved nodes focus only", async ({
    page,
  }) => {
    const vaultName = TEST_VAULT;
    ...
    await expect(page.getByTestId("graph-preview-sheet")).toContainText(
      "Preview body for Project Plan",
    );
    expect(
  ...
  test("keyboard search, preview action, and escape keep canvas graph accessible", async ({
    page,
  }) => {
    const vaultName = TEST_VAULT;
    await mockGraphApi(page, async (route) =>
    ...
    await mockNoteApi(page, async (route) =>
      json(route, notePayload("project-plan", "Project Plan")),
    );
    ...
    await page
      .getByRole("searchbox", { name: "Search graph nodes" })
      .fill("project");
    await page.getByRole("button", { name: /Focus Project Plan/ }).click();
    await expect(page.getByTestId("graph-focus-status")).toContainText(
      "Project Plan",
    );
    await expect(page.getByTestId("graph-a11y-status")).toContainText(
      /Project Plan/,
    );
  ...
  test("handles graph failures, malformed payloads, cap status, and preview failures", async ({
    page,
  }) => {
    const vaultName = TEST_VAULT;
    ...
    await page.unroute("**/api/v1/vault/*/graph");
    await mockGraphApi(page, async (route) =>
      json(route, graphWithLowImportanceNodes(325)),
    );
    await mockNoteApi(page, async (route) =>
      route.fulfill({ status: 500, body: "preview boom" }),
    );
    await page.goto(graphPath(vaultName));
    await settleGraph(page);
    await expect(page.getByTestId("graph-cap-status")).toHaveCount(0);
    ...
    await page.getByRole("button", { name: /Focus Project Plan/ }).click();
    await page.getByRole("button", { name: /Preview focused note/ }).click();
    await expect(page.getByTestId("graph-preview-error")).toContainText(
      /preview.*500/i,
    );
  });

  test("keeps zoom controls usable on mobile viewports and dark theme surfaces dark", async ({
    page,
  }, testInfo) => {
    const vaultName = TEST_VAULT;
...
async function settleGraph(page: Page, ticks = 180) {
  ...
  await page.waitForFunction(
    () =>
      Boolean(
        (window as typeof window & { __pkmGraphTest?: TestApi }).__pkmGraphTest
          ?.settle,
      ),
    null,
    { timeout: 2000 },
  ...
  await page.evaluate(async (settleTicks) => {
    await (
      window as typeof window & { __pkmGraphTest: TestApi }
    ).__pkmGraphTest.settle(settleTicks);
  }, ticks);
}
...
async function graphEdge(
  ...
  return page.evaluate(
    ([s, t, edgeType]) =>
      (
        window as typeof window & { __pkmGraphTest: TestApi }
      ).__pkmGraphTest.getEdge(s, t, edgeType || undefined),
    [source, target, type ?? ""],
  );
}
...
async function graphCounts(page: Page) {
  return page.evaluate(() =>
    (
      window as typeof window & { __pkmGraphTest: TestApi }
    ).__pkmGraphTest.getRenderedCounts(),
  );
}
...
async function graphLabels(page: Page) {
  return page.evaluate(() =>
    (
      window as typeof window & { __pkmGraphTest: TestApi }
    ).__pkmGraphTest.getRenderedLabels(),
  );
}
...
async function graphForceOptions(page: Page) {
  return page.evaluate(() =>
    (
      window as typeof window & { __pkmGraphTest: TestApi }
    ).__pkmGraphTest.getForceOptions(),
  );
}
...
async function graphSimulationState(page: Page) {
  return page.evaluate(() =>
    (
      window as typeof window & { __pkmGraphTest: TestApi }
    ).__pkmGraphTest.getSimulationState(),
  );
}
...
async function graphNodeStyle(page: Page, id: string) {
  return page.evaluate(
    (nodeId) =>
      (
        window as typeof window & { __pkmGraphTest: TestApi }
      ).__pkmGraphTest.getNodeStyle(nodeId),
    id,
  );
}
...
async function focusState(page: Page, id: string) {
  return page.evaluate(
    (nodeId) =>
      (
        window as typeof window & { __pkmGraphTest: TestApi }
      ).__pkmGraphTest.getFocusState(nodeId),
    id,
  );
}

async function setRange(page: Page, name: string, value: string) {
  await page.getByLabel(name).evaluate((input, nextValue) => {
...
async function clickNode(
  ...
  await page.mouse.click(
    box!.x + node!.x * transform.k + transform.x,
    box!.y + node!.y * transform.k + transform.y,
  );
  ...
  for (const candidate of candidates) {
    const hit = await page.evaluate(
      ({ x, y, left, top }) =>
        (
          window as typeof window & { __pkmGraphTest: TestApi }
        ).__pkmGraphTest.hitTest(x - left, y - top),
...
function graphWithLowImportanceNodes(count: number): GraphPayload {
  ...
  return {
    nodes: [
      ...enrichedGraphFixture.nodes.filter(
        (node) => node.id !== "missing-roadmap",
      ),
      ...lowNodes,
    ],
