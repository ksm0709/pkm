...
test.describe('note contrast', () => {
  for (const mode of ['light', 'dark', 'auto-light', 'auto-dark'] as ThemeMode[]) {
    test(`${mode} keeps note and editor surfaces readable`, async ({ page }) => {
...
async function assertContrast(page: Page, selector: string, mode: ThemeMode) {
  ...
  expect(metrics.contrast, `${selector} ${mode} contrast`).toBeGreaterThanOrEqual(4.5);

  if (mode === 'dark' || mode === 'auto-dark') {
    expect(metrics.bgLum, `${selector} ${mode} dark background`).toBeLessThan(0.25);
    expect(metrics.fgLum, `${selector} ${mode} light text`).toBeGreaterThan(0.55);
  } else {
    expect(metrics.bgLum, `${selector} ${mode} light background`).toBeGreaterThan(0.75);
