...
test.describe("PWA installability contract", () => {
  test("serves a web app manifest with install metadata and icons", async ({
    request,
  }) => {
  ...
  test("document head advertises install metadata", async ({ page }) => {
    ...
    const registrationScript = await page
      .locator("script")
      .evaluateAll((scripts) =>
        scripts.some((script) =>
          script.textContent?.includes("service-worker.js"),
        ),
      );
  ...
  test("requests portrait orientation lock when supported", async ({
    page,
  }) => {
